...
class PAF32s(nn.Module):
    ...
    def forward(self, x):
        ...
        pose = pose[:, :, 5:5 + np.ceil(x.size()[2]/32).astype(np.int64), 5:5 + np.ceil(x.size()[3]/32).astype(np.int64)].contiguous()

        paf = self.poserelu1(self.poseconv1(h))
        paf = self.poserelu2(self.poseconv2(paf))
        paf = self.poserelu3(self.poseconv3(paf))
        paf = self.poserelu4(self.poseconv4(paf))
        paf = self.poseconv5(paf)
        paf = paf[:, :, 5:5 + np.ceil(x.size()[2]/32).astype(np.int64), 5:5 + np.ceil(x.size()[3]/32).astype(np.int64)].contiguous()
